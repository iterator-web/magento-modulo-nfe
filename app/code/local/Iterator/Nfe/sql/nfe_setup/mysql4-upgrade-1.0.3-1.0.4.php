...
try {
$installer->run("
    ALTER TABLE {$installer->getTable('sales/order')} ADD COLUMN `nfe_numero` INT(9) UNSIGNED ZEROFILL NULL;
    ALTER TABLE {$installer->getTable('sales/order')} ADD COLUMN `nfe_emissao` DATETIME NULL;
");
} catch (Exception $e) {
